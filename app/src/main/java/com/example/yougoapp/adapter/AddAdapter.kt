...
import com.example.yougoapp.databinding.ItemScheduleBinding
import com.example.yougoapp.response.PoseItem
import com.example.yougoapp.ui.pose.DetailPoseActivity
import java.util.Locale

class AddAdapter(private var poseData: List<PoseItem>) :
    RecyclerView.Adapter<AddAdapter.PoseViewHolder>(), Filterable {

    private var filteredPose: List<PoseItem> = poseData

    inner class PoseViewHolder(private val binding: ItemScheduleBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AddAdapter.PoseViewHolder {
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (!constraint.isNullOrEmpty()) {
                    ...
                    for (poseData in poseData) {
                        if (poseData.title?.toLowerCase(Locale.ROOT)
